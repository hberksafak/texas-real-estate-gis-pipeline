...
def create_candidate_grid(dallas_county: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    county_geometry = (
        county_analysis.geometry.union_all()
        if hasattr(county_analysis.geometry, "union_all")
        else county_analysis.geometry.unary_union
    )
...
def failed_rules_for_candidate(row: pd.Series) -> list[str]:
    ...
    if row["area_ratio_to_full_grid"] < EDGE_FRAGMENT_MIN_AREA_RATIO:
